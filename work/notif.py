...
# AI のキャラ設定
seikaku = "お便りを受け付けそれについてトークを行ってください"

seikakuYoru = "中二病である四国めたんでニュースを読み上げるラジオ番組のパーソナリティとなってください"
...
def merge_wav_to_mp3(files, file_list):
    ...
    command = [
        ...
        "-i", WAV_DIR + file_list,
        "-c:a", "libmp3lame",
        "-q:a", "2",  # MP3の品質設定（2は高品質、値が小さいほど高品質）
        output_mp3_path
    ]
...
async def voicevoxRequest(text):
    ...
    # mp3File = f"{file_name.replace('.wav', '.mp3')}"
    # convert_wav_to_mp3(WAV_DIR + file_name, WAV_DIR + mp3File)

    # # クライアントへ通知
    # data = {"action": "fileCreate", "value": mp3File}
    # await broadcast(data)
